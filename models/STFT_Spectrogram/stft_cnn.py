...
class STFT_Three_Layer_CNN_Pro(nn.Module):
    def __init__(self, eeg_channel=14, dropout_prob=0.5) -> None:
        super().__init__()
        first_layer_out_channel = 16
        output_channels = 128
        ...
        self.conv_last = nn.Conv2d(in_channels=first_layer_out_channel * 3, out_channels=output_channels * 3, 
                                   kernel_size=(3, 3), stride=1, 
                                   padding=(int(math.ceil((3 - 1) / 2)), int(math.ceil((3 - 1) / 2))))
        
        self.conv_last2 = nn.Conv2d(in_channels=output_channels * 3, out_channels=output_channels, 
                                   kernel_size=(3, 3), stride=1, 
                                   padding=(int(math.ceil((3 - 1) / 2)), int(math.ceil((3 - 1) / 2))))
        ...
        self.bn_temporal = nn.BatchNorm2d(first_layer_out_channel)
        self.bn_spatial_temporal = nn.BatchNorm2d(first_layer_out_channel)
        self.bn_last = nn.BatchNorm2d(output_channels*3)
        self.bn_last2 = nn.BatchNorm2d(output_channels)
    ...
    def forward(self, x):
        ...
        # Apply final convolutional layer with BatchNorm
        x = self.activation_func(self.bn_last(self.conv_last(x)))
        x = self.activation_func(self.bn_last2(self.conv_last2(x)))
